...
public partial class SpeechToTextViewModel : BaseViewModel
{
    ...
    [ObservableProperty]
    string? recognitionText = "Start talking!";
    ...
    public SpeechToTextViewModel(ISpeechToText speechToText)
    {
    ...
    async Task SetLocales(CancellationToken token)
    {
        ...
        this.CurrentLocale = Locales.FirstOrDefault(x => x.Language is defaultLanguage or defaultLanguage_android or defaultLanguage_tizen) ?? Locales.FirstOrDefault();
    }
    ...
    */
    ...
    async Task StartListen(CancellationToken cancellationToken)
    {
        ...
        await speechToText.StartListenAsync(CultureInfo.GetCultureInfo(CurrentLocale?.Language ?? defaultLanguage),
            cancellationToken);
